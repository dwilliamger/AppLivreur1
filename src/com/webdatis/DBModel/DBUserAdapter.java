...
public class DBUserAdapter {
	...
	private static final String DATABASE_CREATE = "create table users (_id integer primary key autoincrement, "
			+ "usermail text not null, " + "password text not null);";
	...
	public long AddUser(String usermail, String password) {
		ContentValues initialValues = new ContentValues();
		initialValues.put(KEY_USERMAIL, usermail);
		initialValues.put(KEY_PASSWORD, password);
		return db.insert(DATABASE_TABLE, null, initialValues);

	}

	public boolean Login(String usermail, String passwords) throws SQLException {
		Cursor mCursor = db.rawQuery("SELECT * FROM " + DATABASE_TABLE
				+ " WHERE usermail=? AND password=?", new String[] { usermail,
				passwords });
		if (mCursor != null) {
